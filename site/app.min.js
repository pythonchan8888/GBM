class ParlayKing{constructor(){this.cache=new Map,this.data={metrics:{},pnlByMonth:[],bankrollSeries:[],recommendations:[],roiHeatmap:[],topSegments:[],settledBets:[],parlayWins:[]},this.filters=this.getFiltersFromURL(),this.version=window.__PK_VERSION||"latest",this.parlayPage=0,this.init()}parseParlayWins(e){return Array.isArray(e)&&0!==e.length?e.map(e=>{var t=parseFloat(e.stake||100),a=parseFloat(e.total_odds||e.odds||1),i=parseFloat(e.payout||t*a),n=parseFloat(e.profit||i-t),s=(e.legs||e.legs_str||"").toString().split(" || ").filter(Boolean).map(e=>{var[e,t]=e.split(" | "),[e,a]=(e||"").split(" vs ").map(e=>(e||"").trim());let i=t||"",n=1;t=i.lastIndexOf("@");return-1!==t&&(n=parseFloat(i.slice(t+1))||1,i=i.slice(0,t).trim()),{home:e,away:a,recommendation:i,odds:n}}),r=this.parseDateTimeSafe(e.window_start||e.start_dt||e.start),l=this.parseDateTimeSafe(e.window_end||e.end_dt||e.end);return{legs:s,legCount:parseInt(e.leg_count||s.length||0),totalOdds:a,stake:t,totalPayout:i,profit:n,returnPercent:n/Math.max(t,1)*100,dateRange:r&&l?this.isSameDay(r,l)?this.formatDate(r):this.formatDate(r)+" - "+this.formatDate(l):"--",startDate:r||null,endDate:l||null}}):[]}async init(){this.showLoading(!0),await this.loadAllData(),this.initializeUI(),this.updateUI(),this.showLoading(!1)}async loadCSV(t,a=!0){window.Papa||await new Promise(e=>{var t=document.createElement("script");t.src="https://unpkg.com/papaparse@5.4.1/papaparse.min.js",t.onload=e,document.head.appendChild(t)});var i="csv_"+t;if(a&&this.cache.has(i))return this.cache.get(i);try{var n,s,r,l=this.cache.has(i),d=a&&l?{"If-None-Match":localStorage.getItem("etag_"+t)||""}:{};let e=await fetch(t+"?t="+Date.now(),{headers:d});if((e=304!==e.status||l?e:await fetch(t+"?t="+Date.now(),{cache:"no-store"})).ok)return n=await e.text(),s=Papa.parse(n,{header:!0,dynamicTyping:!0,skipEmptyLines:!0}).data,this.cache.set(i,s),(r=e.headers.get("ETag"))&&localStorage.setItem("etag_"+t,r),s;throw new Error(`Failed to load ${t}: `+e.status)}catch(e){return console.warn(`Failed to load ${t}:`,e),this.cache.get(i)||[]}}async loadAllData(){try{var[e,t,a,i,n,s,r,l,d]=await Promise.all([this.loadCSV("metrics.csv"),this.loadCSV("pnl_by_month.csv"),this.loadCSV("bankroll_series_90d.csv"),this.loadCSV("latest_recommendations.csv"),this.loadCSV("roi_heatmap.csv"),this.loadCSV("top_segments.csv"),this.loadCSV("settled_bets.csv"),this.loadCSV("parlay_wins.csv").catch(()=>[]),this.loadCSV("unified_games.csv").catch(e=>(console.warn("Unified games CSV not available:",e),[]))]),o=(this.data={metrics:this.parseMetrics(e),pnlByMonth:t,bankrollSeries:this.parseBankrollSeries(a),recommendations:this.parseRecommendations(i),roiHeatmap:n,topSegments:s,settledBets:this.parseSettledBets(r),unifiedGames:this.parseUnifiedGames(d)},this.parseParlayWins(l||[]));this.data.parlayWins=o&&0<o.length?o:this.calculateParlayWinsFromBets(),this.populateFilterOptions(),document.querySelectorAll(".lazy-load").forEach(e=>{e.classList.add("loaded")})}catch(e){console.error("Failed to load data:",e),this.showError("Failed to load dashboard data. Please try refreshing the page.")}}parseMetrics(e){let t={};return Array.isArray(e)&&e.forEach(e=>{e&&e.metric&&null!=e.value&&""!==e.value&&(t[e.metric]=e.value)}),t}parseDateTimeSafe(e){if(!e)return null;if(e instanceof Date)return isNaN(e)?null:e;try{var t,a,i,n,s,r,l,d,o,c=String(e).replace(" ","T"),m=new Date(c);return isNaN(m)?(m=new Date(c+"Z"),isNaN(m)?([t,a]=String(e).split(" "),[i,n,s]=t.split("-").map(Number),[r=0,l=0,d=0]=(a||"").split(":").map(Number),o=new Date(i,(n||1)-1,s||1,r,l,d),isNaN(o)?null:o):m):m}catch(e){return null}}parseGmt8(t){if(!t)return null;try{var[e,a="00:00:00"]=String(t).split(" "),[i,n,s]=e.split("-").map(Number),[r,l,d]=a.split(":").map(Number),o=Date.UTC(i,(n||1)-1,s||1,(r||0)-8,l||0,d||0);return new Date(o)}catch(e){return this.parseDateTimeSafe(t)}}parseBankrollSeries(e){return Array.isArray(e)?e.filter(e=>e&&e.dt_gmt8&&void 0!==e.cum_bankroll).map(e=>({date:this.parseDateTimeSafe(e.dt_gmt8),bankroll:parseFloat(e.cum_bankroll)||0})).sort((e,t)=>e.date-t.date):[]}parseRecommendations(e){return Array.isArray(e)?e.filter(e=>e&&e.dt_gmt8&&e.home&&e.away).map(e=>{var t=this.parseGmt8(e.dt_gmt8)||this.parseDateTimeSafe(e.dt_gmt8);return t?{datetime:t,league:e.league||"",home:e.home,away:e.away,recommendation:e.rec_text||e.recommendation||"",line:parseFloat(e.line)||0,odds:parseFloat(e.odds)||0,ev:parseFloat(e.ev)||0,confidence:e.confidence||"Medium",kingsCall:e.kings_call_insight||"No additional insights available."}:(console.warn("Invalid date for recommendation: "+e.dt_gmt8),null)}).filter(Boolean).sort((e,t)=>{var a=e.datetime-t.datetime;return console.log(`Sorting: ${e.home} vs ${e.away} (${e.datetime}) vs ${t.home} vs ${t.away} (${t.datetime}) = `+a),a}):[]}parseSettledBets(e){return Array.isArray(e)?e.filter(e=>e&&e.home&&e.away).map(e=>{var t=(e.bet_type_refined_ah||e.bet_type||"").toString(),a=parseFloat(e.line_betted_on_refined||e.line||0),i=parseFloat(e.odds_betted_on_refined||e.odds||0),n=parseFloat(e.pl||0),s=(e.status||"").toString().toLowerCase(),s=0<n||"won"===s||"win"===s,r=0===n,l=!r&&i||1,d=t.toLowerCase();let o;return o=d.includes("away")?e.away+" "+(0<=a?"+":"")+(isFinite(a)?a.toFixed(2):"0.00"):d.includes("home")?e.home+" "+(0<=a?"+":"")+(isFinite(a)?a.toFixed(2):"0.00"):t?t+" "+(0<=a?"+":"")+(isFinite(a)?a.toFixed(2):"0.00"):"AH "+(0<=a?"+":"")+(isFinite(a)?a.toFixed(2):"0.00"),{fixture_id:e.fixture_id,league:e.league||"",home:e.home,away:e.away,home_score:parseInt(e.home_score||0),away_score:parseInt(e.away_score||0),bet_type:t,line:a,odds:i||1,effectiveOdds:l,stake:parseFloat(e.stake||0),pl:n,isWin:s,isPush:r,recommendation:o,status:e.status||"",datetime:this.parseGmt8(e.dt_gmt8)||this.parseDateTimeSafe(e.dt_gmt8)}}).sort((e,t)=>(t.datetime||0)-(e.datetime||0)):[]}parseUnifiedGames(e){return Array.isArray(e)&&0!==e.length?e.filter(e=>e&&e.datetime_gmt8&&e.home_name&&e.away_name).map(e=>{var t=this.parseGmt8(e.datetime_gmt8)||this.parseDateTimeSafe(e.datetime_gmt8);return t?{datetime:t,league:e.league||"",leagueShort:e.league_short||e.league||"",leagueFlag:e.league_flag||"⚽",leagueColor:e.league_color||"#666666",home:e.home_name,away:e.away_name,odds1:parseFloat(e.odds_1)||0,oddsX:parseFloat(e.odds_x)||0,odds2:parseFloat(e.odds_2)||0,tier:parseInt(e.league_tier)||3,competitionType:e.competition_type||"league",isFuture:"True"===e.is_future||!0===e.is_future||"true"===e.is_future,status:e.status||"",hasRecommendation:"True"===e.has_recommendation||!0===e.has_recommendation||"true"===e.has_recommendation,recText:e.rec_text||"",line:parseFloat(e.line)||0,recOdds:parseFloat(e.rec_odds)||0,ev:parseFloat(e.ev)||0,confidence:e.confidence||"",kingsCall:e.kings_call||"",kingsCallAgreement:e.kings_call_agreement||"",primarySignal:e.primary_signal||"",secondarySignal:e.secondary_signal||""}:(console.warn("Invalid date for game: "+e.datetime_gmt8),null)}).filter(Boolean).sort((e,t)=>e.datetime-t.datetime):(console.log("No unified games data available - will use fallback recommendations"),[])}calculateParlayWinsFromBets(){var e=this.data.settledBets||[],e=this.groupBetsByWindow(e,12);let a=[];return e.forEach(e=>{var t=e.filter(e=>e.isWin||e.isPush);for(let e=3;e<=Math.min(6,t.length);e++)this.getCombinations(t,e).sort((e,t)=>this.calculateParlayOdds(t)-this.calculateParlayOdds(e)).slice(0,2).forEach(e=>a.push(this.createParlayItem(e)))}),a.sort((e,t)=>t.totalPayout-e.totalPayout).slice(0,6)}groupBetsByWindow(e,t=0){let a=new Map,s=(e,t,a,i)=>new Date(Date.UTC(e,t-1,a,i-8,0,0)),i=e=>{var e=new Date(e.getTime()+288e5),t=e.getUTCFullYear(),a=e.getUTCMonth()+1,i=e.getUTCDate(),n=e.getUTCHours();return 5<=n&&n<17?s(t,a,i,5):17<=n?s(t,a,i,17):(n=new Date(e.getTime()-864e5),s(n.getUTCFullYear(),n.getUTCMonth()+1,n.getUTCDate(),17))};return e.forEach(e=>{var t;e.datetime&&(t=i(e.datetime).getTime(),a.has(t)||a.set(t,[]),a.get(t).push(e))}),Array.from(a.values())}getCombinations(a,e){if(1===e)return a.map(e=>[e]);if(e>a.length)return[];let i=[];for(let t=0;t<a.length-e+1;t++)this.getCombinations(a.slice(t+1),e-1).forEach(e=>{i.push([a[t],...e])});return i}calculateParlayOdds(e){return e.reduce((e,t)=>e*(t.effectiveOdds||t.odds||1),1)}createParlayItem(e){var t=this.calculateParlayOdds(e),a=100*t,i=a-100;return{legs:e,legCount:e.length,totalOdds:t,stake:100,totalPayout:a,profit:i,returnPercent:i/100*100,dateRange:this.getParlayDateRange(e)}}getParlayDateRange(e){var e=e.map(e=>e.datetime).sort(),t=e[0],e=e[e.length-1];return this.isSameDay(t,e)?this.formatDate(t):this.formatDate(t)+" - "+this.formatDate(e)}isSameDay(e,t){return e.toDateString()===t.toDateString()}createCustomChart(g,p,u={}){let v=document.getElementById(g);if(v)if(v.innerHTML="",p&&Array.isArray(p)&&0!==p.length){let h=p.filter(e=>e&&"number"==typeof e.value&&!isNaN(e.value));if(0!==h.length){var g=document.createElementNS("http://www.w3.org/2000/svg","svg"),p=(g.setAttribute("class","chart-svg"),g.setAttribute("viewBox","0 0 800 400"),g.setAttribute("preserveAspectRatio","xMidYMid meet"),document.createElementNS("http://www.w3.org/2000/svg","defs")),f=document.createElementNS("http://www.w3.org/2000/svg","linearGradient"),y=(f.setAttribute("id","chartGradient"),f.setAttribute("x1","0%"),f.setAttribute("y1","0%"),f.setAttribute("x2","0%"),f.setAttribute("y2","100%"),document.createElementNS("http://www.w3.org/2000/svg","stop")),w=(y.setAttribute("offset","0%"),y.setAttribute("stop-color","#FF7A45"),y.setAttribute("stop-opacity","0.3"),document.createElementNS("http://www.w3.org/2000/svg","stop"));w.setAttribute("offset","100%"),w.setAttribute("stop-color","#FF7A45"),w.setAttribute("stop-opacity","0.05"),f.appendChild(y),f.appendChild(w),p.appendChild(f),g.appendChild(p);let a={top:20,right:30,bottom:40,left:60},i=800-a.left-a.right,n=400-a.top-a.bottom,s=e=>a.left+e/Math.max(h.length-1,1)*i,t=Math.min(...h.map(e=>e.value)),r=Math.max(...h.map(e=>e.value)),l=.1*(r-t),d=e=>a.top+n-(e-(t-l))/(r+l-(t-l))*n;var b=document.createElementNS("http://www.w3.org/2000/svg","g");for(let t=0;t<=5;t++){var x=a.top+t/5*n;let e=document.createElementNS("http://www.w3.org/2000/svg","line");e.setAttribute("x1",a.left),e.setAttribute("y1",x),e.setAttribute("x2",a.left+i),e.setAttribute("y2",x),e.setAttribute("class","chart-grid-line"),b.appendChild(e)}g.appendChild(b);let o=`M ${a.left} `+(a.top+n);h.forEach((e,t)=>{o+=` L ${s(t)} `+d(e.value)}),o+=` L ${a.left+i} ${a.top+n} Z`;y=document.createElementNS("http://www.w3.org/2000/svg","path");y.setAttribute("d",o),y.setAttribute("class","chart-area"),g.appendChild(y);let c=`M ${s(0)} `+d(h[0].value),e=(h.slice(1).forEach((e,t)=>{c+=` L ${s(t+1)} `+d(e.value)}),document.createElementNS("http://www.w3.org/2000/svg","path")),m=(e.setAttribute("d",c),e.setAttribute("class","chart-line"),g.appendChild(e),document.createElementNS("http://www.w3.org/2000/svg","g"));h.forEach((t,e)=>{var a=document.createElementNS("http://www.w3.org/2000/svg","circle");a.setAttribute("cx",s(e)),a.setAttribute("cy",d(t.value)),a.setAttribute("r","4"),a.setAttribute("class","chart-point"),a.setAttribute("data-index",e),a.addEventListener("mouseenter",e=>this.showChartTooltip(e,t,v)),a.addEventListener("mouseleave",()=>this.hideChartTooltip(v)),m.appendChild(a)}),g.appendChild(m);var E=document.createElementNS("http://www.w3.org/2000/svg","g");for(let e=0;e<=5;e++){var C=t+(r-t)*(e/5),S=document.createElementNS("http://www.w3.org/2000/svg","text");S.setAttribute("x",a.left-10),S.setAttribute("y",a.top+n-e/5*n+4),S.setAttribute("text-anchor","end"),S.setAttribute("class","chart-axis-text"),S.textContent=u.formatY?u.formatY(C):C.toFixed(1),E.appendChild(S)}g.appendChild(E),v.appendChild(g);w=document.createElement("div");return w.className="chart-tooltip",v.appendChild(w),g}v.innerHTML='<div class="chart-error">No valid data points</div>'}else v.innerHTML='<div class="chart-error">No data available</div>'}showChartTooltip(e,t,a){a=a.querySelector(".chart-tooltip");a.innerHTML=`
        ...
        `}renderGameCard(e,t){var a=this.getSignalIcon(e.primarySignal),i=e.hasRecommendation,n=this.formatGameTime(e.datetime),s=this.formatOddsCompact(e);return`
            <div class="game-card" 
                 data-game-index="${t}"
