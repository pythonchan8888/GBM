class ParlayKing{constructor(){this.cache=new Map,this.data={metrics:{},pnlByMonth:[],bankrollSeries:[],recommendations:[],roiHeatmap:[],topSegments:[],settledBets:[],parlayWins:[]},this.filters=this.getFiltersFromURL(),this.version=window.__PK_VERSION||"latest",this.parlayPage=0,this.init()}parseParlayWins(e){return Array.isArray(e)&&0!==e.length?e.map(e=>{var t=parseFloat(e.stake||100),a=parseFloat(e.total_odds||e.odds||1),i=parseFloat(e.payout||t*a),n=parseFloat(e.profit||i-t),r=(e.legs||e.legs_str||"").toString().split(" || ").filter(Boolean).map(e=>{var[e,t]=e.split(" | "),[e,a]=(e||"").split(" vs ").map(e=>(e||"").trim());let i=t||"",n=1;t=i.lastIndexOf("@");return-1!==t&&(n=parseFloat(i.slice(t+1))||1,i=i.slice(0,t).trim()),{home:e,away:a,recommendation:i,odds:n}}),s=this.parseDateTimeSafe(e.window_start||e.start_dt||e.start),l=this.parseDateTimeSafe(e.window_end||e.end_dt||e.end);return{legs:r,legCount:parseInt(e.leg_count||r.length||0),totalOdds:a,stake:t,totalPayout:i,profit:n,returnPercent:n/Math.max(t,1)*100,dateRange:s&&l?this.isSameDay(s,l)?this.formatDate(s):this.formatDate(s)+" - "+this.formatDate(l):"--",startDate:s||null,endDate:l||null}}):[]}async init(){this.showLoading(!0),await this.loadAllData(),this.initializeUI(),this.updateUI(),this.showLoading(!1)}async loadCSV(t,a=!0){window.Papa||await new Promise(e=>{var t=document.createElement("script");t.src="https://unpkg.com/papaparse@5.4.1/papaparse.min.js",t.onload=e,document.head.appendChild(t)});var i="csv_"+t;if(a&&this.cache.has(i))return this.cache.get(i);try{var n,r,s,l=this.cache.has(i),d=a&&l?{"If-None-Match":localStorage.getItem("etag_"+t)||""}:{};let e=await fetch(t+"?t="+Date.now(),{headers:d});if((e=304!==e.status||l?e:await fetch(t+"?t="+Date.now(),{cache:"no-store"})).ok)return n=await e.text(),r=Papa.parse(n,{header:!0,dynamicTyping:!0,skipEmptyLines:!0}).data,this.cache.set(i,r),(s=e.headers.get("ETag"))&&localStorage.setItem("etag_"+t,s),r;throw new Error(`Failed to load ${t}: `+e.status)}catch(e){return console.warn(`Failed to load ${t}:`,e),this.cache.get(i)||[]}}async loadAllData(){try{var[e,t,a,i,n,r,s,l,d]=await Promise.all([this.loadCSV("metrics.csv"),this.loadCSV("pnl_by_month.csv"),this.loadCSV("bankroll_series_90d.csv"),this.loadCSV("latest_recommendations.csv"),this.loadCSV("roi_heatmap.csv"),this.loadCSV("top_segments.csv"),this.loadCSV("settled_bets.csv"),this.loadCSV("parlay_wins.csv").catch(()=>[]),this.loadCSV("unified_games.csv").catch(e=>(console.warn("Unified games CSV not available:",e),[]))]),o=(this.data={metrics:this.parseMetrics(e),pnlByMonth:t,bankrollSeries:this.parseBankrollSeries(a),recommendations:this.parseRecommendations(i),roiHeatmap:n,topSegments:r,settledBets:this.parseSettledBets(s),unifiedGames:this.parseUnifiedGames(d)},this.parseParlayWins(l||[]));this.data.parlayWins=o&&0<o.length?o:this.calculateParlayWinsFromBets(),this.populateFilterOptions(),document.querySelectorAll(".lazy-load").forEach(e=>{e.classList.add("loaded")})}catch(e){console.error("Failed to load data:",e),this.showError("Failed to load dashboard data. Please try refreshing the page.")}}parseMetrics(e){let t={};return Array.isArray(e)&&e.forEach(e=>{e&&e.metric&&null!=e.value&&""!==e.value&&(t[e.metric]=e.value)}),t}parseDateTimeSafe(e){if(!e)return null;if(e instanceof Date)return isNaN(e)?null:e;try{var t,a,i,n,r,s,l,d,o,c=String(e).replace(" ","T"),m=new Date(c);return isNaN(m)?(m=new Date(c+"Z"),isNaN(m)?([t,a]=String(e).split(" "),[i,n,r]=t.split("-").map(Number),[s=0,l=0,d=0]=(a||"").split(":").map(Number),o=new Date(i,(n||1)-1,r||1,s,l,d),isNaN(o)?null:o):m):m}catch(e){return null}}parseGmt8(t){if(!t)return null;try{var[e,a="00:00:00"]=String(t).split(" "),[i,n,r]=e.split("-").map(Number),[s,l,d]=a.split(":").map(Number),o=Date.UTC(i,(n||1)-1,r||1,(s||0)-8,l||0,d||0);return new Date(o)}catch(e){return this.parseDateTimeSafe(t)}}parseBankrollSeries(e){return Array.isArray(e)?e.filter(e=>e&&e.dt_gmt8&&void 0!==e.cum_bankroll).map(e=>({date:this.parseDateTimeSafe(e.dt_gmt8),bankroll:parseFloat(e.cum_bankroll)||0})).sort((e,t)=>e.date-t.date):[]}parseRecommendations(e){return Array.isArray(e)?e.filter(e=>e&&e.dt_gmt8&&e.home&&e.away).map(e=>{var t=this.parseGmt8(e.dt_gmt8)||this.parseDateTimeSafe(e.dt_gmt8);return t?{datetime:t,league:e.league||"",home:e.home,away:e.away,recommendation:e.rec_text||e.recommendation||"",line:parseFloat(e.line)||0,odds:parseFloat(e.odds)||0,ev:parseFloat(e.ev)||0,confidence:e.confidence||"Medium",kingsCall:e.kings_call_insight||"No additional insights available."}:(console.warn("Invalid date for recommendation: "+e.dt_gmt8),null)}).filter(Boolean).sort((e,t)=>{var a=e.datetime-t.datetime;return console.log(`Sorting: ${e.home} vs ${e.away} (${e.datetime}) vs ${t.home} vs ${t.away} (${t.datetime}) = `+a),a}):[]}parseSettledBets(e){return Array.isArray(e)?e.filter(e=>e&&e.home&&e.away).map(e=>{var t=(e.bet_type_refined_ah||e.bet_type||"").toString(),a=parseFloat(e.line_betted_on_refined||e.line||0),i=parseFloat(e.odds_betted_on_refined||e.odds||0),n=parseFloat(e.pl||0),r=(e.status||"").toString().toLowerCase(),r=0<n||"won"===r||"win"===r,s=0===n,l=!s&&i||1,d=t.toLowerCase();let o;return o=d.includes("away")?e.away+" "+(0<=a?"+":"")+(isFinite(a)?a.toFixed(2):"0.00"):d.includes("home")?e.home+" "+(0<=a?"+":"")+(isFinite(a)?a.toFixed(2):"0.00"):t?t+" "+(0<=a?"+":"")+(isFinite(a)?a.toFixed(2):"0.00"):"AH "+(0<=a?"+":"")+(isFinite(a)?a.toFixed(2):"0.00"),{fixture_id:e.fixture_id,league:e.league||"",home:e.home,away:e.away,home_score:parseInt(e.home_score||0),away_score:parseInt(e.away_score||0),bet_type:t,line:a,odds:i||1,effectiveOdds:l,stake:parseFloat(e.stake||0),pl:n,isWin:r,isPush:s,recommendation:o,status:e.status||"",datetime:this.parseGmt8(e.dt_gmt8)||this.parseDateTimeSafe(e.dt_gmt8)}}).sort((e,t)=>(t.datetime||0)-(e.datetime||0)):[]}parseUnifiedGames(e){return Array.isArray(e)&&0!==e.length?e.filter(e=>e&&e.datetime_gmt8&&e.home_name&&e.away_name).map(e=>{var t=this.parseGmt8(e.datetime_gmt8)||this.parseDateTimeSafe(e.datetime_gmt8);return t?{datetime:t,league:e.league||"",leagueShort:e.league_short||e.league||"",leagueFlag:e.league_flag||"⚽",leagueColor:e.league_color||"#666666",home:e.home_name,away:e.away_name,odds1:parseFloat(e.odds_1)||0,oddsX:parseFloat(e.odds_x)||0,odds2:parseFloat(e.odds_2)||0,tier:parseInt(e.league_tier)||3,competitionType:e.competition_type||"league",isFuture:"True"===e.is_future||!0===e.is_future||"true"===e.is_future,status:e.status||"",hasRecommendation:"True"===e.has_recommendation||!0===e.has_recommendation||"true"===e.has_recommendation,recText:e.rec_text||"",line:parseFloat(e.line)||0,recOdds:parseFloat(e.rec_odds)||0,ev:parseFloat(e.ev)||0,confidence:e.confidence||"",kingsCall:e.kings_call||"",kingsCallAgreement:e.kings_call_agreement||"",primarySignal:e.primary_signal||"",secondarySignal:e.secondary_signal||""}:(console.warn("Invalid date for game: "+e.datetime_gmt8),null)}).filter(Boolean).sort((e,t)=>e.datetime-t.datetime):(console.log("No unified games data available - will use fallback recommendations"),[])}calculateParlayWinsFromBets(){var e=this.data.settledBets||[],e=this.groupBetsByWindow(e,12);let a=[];return e.forEach(e=>{var t=e.filter(e=>e.isWin||e.isPush);for(let e=3;e<=Math.min(6,t.length);e++)this.getCombinations(t,e).sort((e,t)=>this.calculateParlayOdds(t)-this.calculateParlayOdds(e)).slice(0,2).forEach(e=>a.push(this.createParlayItem(e)))}),a.sort((e,t)=>t.totalPayout-e.totalPayout).slice(0,6)}groupBetsByWindow(e,t=0){let a=new Map,r=(e,t,a,i)=>new Date(Date.UTC(e,t-1,a,i-8,0,0)),i=e=>{var e=new Date(e.getTime()+288e5),t=e.getUTCFullYear(),a=e.getUTCMonth()+1,i=e.getUTCDate(),n=e.getUTCHours();return 5<=n&&n<17?r(t,a,i,5):17<=n?r(t,a,i,17):(n=new Date(e.getTime()-864e5),r(n.getUTCFullYear(),n.getUTCMonth()+1,n.getUTCDate(),17))};return e.forEach(e=>{var t;e.datetime&&(t=i(e.datetime).getTime(),a.has(t)||a.set(t,[]),a.get(t).push(e))}),Array.from(a.values())}getCombinations(a,e){if(1===e)return a.map(e=>[e]);if(e>a.length)return[];let i=[];for(let t=0;t<a.length-e+1;t++)this.getCombinations(a.slice(t+1),e-1).forEach(e=>{i.push([a[t],...e])});return i}calculateParlayOdds(e){return e.reduce((e,t)=>e*(t.effectiveOdds||t.odds||1),1)}createParlayItem(e){var t=this.calculateParlayOdds(e),a=100*t,i=a-100;return{legs:e,legCount:e.length,totalOdds:t,stake:100,totalPayout:a,profit:i,returnPercent:i/100*100,dateRange:this.getParlayDateRange(e)}}getParlayDateRange(e){var e=e.map(e=>e.datetime).sort(),t=e[0],e=e[e.length-1];return this.isSameDay(t,e)?this.formatDate(t):this.formatDate(t)+" - "+this.formatDate(e)}isSameDay(e,t){return e.toDateString()===t.toDateString()}createCustomChart(g,p,u={}){let v=document.getElementById(g);if(v)if(v.innerHTML="",p&&Array.isArray(p)&&0!==p.length){let h=p.filter(e=>e&&"number"==typeof e.value&&!isNaN(e.value));if(0!==h.length){var g=document.createElementNS("http://www.w3.org/2000/svg","svg"),p=(g.setAttribute("class","chart-svg"),g.setAttribute("viewBox","0 0 800 400"),g.setAttribute("preserveAspectRatio","xMidYMid meet"),document.createElementNS("http://www.w3.org/2000/svg","defs")),f=document.createElementNS("http://www.w3.org/2000/svg","linearGradient"),y=(f.setAttribute("id","chartGradient"),f.setAttribute("x1","0%"),f.setAttribute("y1","0%"),f.setAttribute("x2","0%"),f.setAttribute("y2","100%"),document.createElementNS("http://www.w3.org/2000/svg","stop")),w=(y.setAttribute("offset","0%"),y.setAttribute("stop-color","#FF7A45"),y.setAttribute("stop-opacity","0.3"),document.createElementNS("http://www.w3.org/2000/svg","stop"));w.setAttribute("offset","100%"),w.setAttribute("stop-color","#FF7A45"),w.setAttribute("stop-opacity","0.05"),f.appendChild(y),f.appendChild(w),p.appendChild(f),g.appendChild(p);let a={top:20,right:30,bottom:40,left:60},i=800-a.left-a.right,n=400-a.top-a.bottom,r=e=>a.left+e/Math.max(h.length-1,1)*i,t=Math.min(...h.map(e=>e.value)),s=Math.max(...h.map(e=>e.value)),l=.1*(s-t),d=e=>a.top+n-(e-(t-l))/(s+l-(t-l))*n;var b=document.createElementNS("http://www.w3.org/2000/svg","g");for(let t=0;t<=5;t++){var x=a.top+t/5*n;let e=document.createElementNS("http://www.w3.org/2000/svg","line");e.setAttribute("x1",a.left),e.setAttribute("y1",x),e.setAttribute("x2",a.left+i),e.setAttribute("y2",x),e.setAttribute("class","chart-grid-line"),b.appendChild(e)}g.appendChild(b);let o=`M ${a.left} `+(a.top+n);h.forEach((e,t)=>{o+=` L ${r(t)} `+d(e.value)}),o+=` L ${a.left+i} ${a.top+n} Z`;y=document.createElementNS("http://www.w3.org/2000/svg","path");y.setAttribute("d",o),y.setAttribute("class","chart-area"),g.appendChild(y);let c=`M ${r(0)} `+d(h[0].value),e=(h.slice(1).forEach((e,t)=>{c+=` L ${r(t+1)} `+d(e.value)}),document.createElementNS("http://www.w3.org/2000/svg","path")),m=(e.setAttribute("d",c),e.setAttribute("class","chart-line"),g.appendChild(e),document.createElementNS("http://www.w3.org/2000/svg","g"));h.forEach((t,e)=>{var a=document.createElementNS("http://www.w3.org/2000/svg","circle");a.setAttribute("cx",r(e)),a.setAttribute("cy",d(t.value)),a.setAttribute("r","4"),a.setAttribute("class","chart-point"),a.setAttribute("data-index",e),a.addEventListener("mouseenter",e=>this.showChartTooltip(e,t,v)),a.addEventListener("mouseleave",()=>this.hideChartTooltip(v)),m.appendChild(a)}),g.appendChild(m);var E=document.createElementNS("http://www.w3.org/2000/svg","g");for(let e=0;e<=5;e++){var C=t+(s-t)*(e/5),S=document.createElementNS("http://www.w3.org/2000/svg","text");S.setAttribute("x",a.left-10),S.setAttribute("y",a.top+n-e/5*n+4),S.setAttribute("text-anchor","end"),S.setAttribute("class","chart-axis-text"),S.textContent=u.formatY?u.formatY(C):C.toFixed(1),E.appendChild(S)}g.appendChild(E),v.appendChild(g);w=document.createElement("div");return w.className="chart-tooltip",v.appendChild(w),g}v.innerHTML='<div class="chart-error">No valid data points</div>'}else v.innerHTML='<div class="chart-error">No data available</div>'}showChartTooltip(e,t,a){a=a.querySelector(".chart-tooltip");a.innerHTML=`
        ...
        `,a.style.left=e.pageX+10+"px",a.style.top=e.pageY-10+"px",a.classList.add("visible")}hideChartTooltip(e){e.querySelector(".chart-tooltip").classList.remove("visible")}getFiltersFromURL(){var e=new URLSearchParams(window.location.search);return{dateRange:e.get("dateRange")||"30",league:e.get("league")||"all",minEV:parseFloat(e.get("minEV"))||0,confidence:e.get("confidence")||"all"}}updateURL(){let a=new URLSearchParams;Object.entries(this.filters).forEach(([e,t])=>{"all"!==t&&0!==t&&"30"!==t&&a.set(e,t)});var e=""+window.location.pathname+(a.toString()?"?"+a.toString():"");window.history.replaceState({},"",e)}populateFilterOptions(){var e=[...new Set(this.data.recommendations.map(e=>e.league))].filter(Boolean).sort();let a=document.getElementById("league-filter");for(;1<a.children.length;)a.removeChild(a.lastChild);e.forEach(e=>{var t=document.createElement("option");t.value=e,t.textContent=e,a.appendChild(t)})}initializeUI(){this.setupEventListeners(),this.setFilterValues(),this.setupChartResize()}setupEventListeners(){var e=document.getElementById("date-range"),e=(e&&e.addEventListener("change",e=>{this.filters.dateRange=e.target.value,this.updateURL(),this.updateUI()}),document.getElementById("league-filter")),e=(e&&e.addEventListener("change",e=>{this.filters.league=e.target.value,this.updateURL(),this.updateUI()}),document.getElementById("min-ev")),e=(e&&e.addEventListener("input",e=>{this.filters.minEV=parseFloat(e.target.value)||0,this.updateURL(),this.updateUI()}),document.getElementById("confidence-filter")),e=(e&&e.addEventListener("change",e=>{this.filters.confidence=e.target.value,this.updateURL(),this.updateUI()}),document.getElementById("reset-filters")),e=(e&&e.addEventListener("click",()=>{this.resetFilters()}),document.querySelectorAll(".toggle-btn[data-mode]").forEach(e=>{e.addEventListener("click",e=>{document.querySelectorAll(".toggle-btn[data-mode]").forEach(e=>e.classList.remove("active")),e.target.classList.add("active"),this.updateChart()})}),document.querySelectorAll(".range-btn[data-range]").forEach(e=>{e.addEventListener("click",e=>{document.querySelectorAll(".range-btn[data-range]").forEach(e=>e.classList.remove("active")),e.target.classList.add("active"),this.updateChart()})}),document.getElementById("export-recommendations"));e&&e.addEventListener("click",()=>{this.exportFilteredRecommendations()}),this.initScheduleFilters()}initScheduleFilters(){document.getElementById("unified-games-container")&&(this.filters.gameTimeFilter||(this.filters.gameTimeFilter="today"),document.querySelectorAll(".filter-tab").forEach(e=>{e.addEventListener("click",e=>{var t=e.target.dataset.filter;document.querySelectorAll(".filter-tab").forEach(e=>e.classList.remove("active")),e.target.classList.add("active"),this.filters.gameTimeFilter=t,this.renderUnifiedSchedule(),this.trackEvent&&this.trackEvent("schedule_filter_changed",{filter:t})})}))}setFilterValues(){var e=document.getElementById("date-range"),e=(e&&(e.value=this.filters.dateRange),document.getElementById("league-filter")),e=(e&&(e.value=this.filters.league),document.getElementById("min-ev")),e=(e&&(e.value=this.filters.minEV),document.getElementById("confidence-filter"));e&&(e.value=this.filters.confidence)}resetFilters(){this.filters={dateRange:"30",league:"all",minEV:0,confidence:"all"},this.setFilterValues(),this.updateURL(),this.updateUI()}updateUI(){this.updateKPIs(),this.updateParlayWins(),this.updateChart(),this.updateRecommendationsTable(),this.renderUnifiedSchedule(),this.updateLastRunStatus()}updateKPIs(){var e,t;this.data&&(t=this.data.metrics||{},parseFloat(t.bets_30d),t=this.data.recommendations?.length||0,(e=document.getElementById("win-rate"))&&(e.textContent=64.91.toFixed(1)+"%"),(e=document.getElementById("roi-performance"))&&(e.textContent=`+${23.81.toFixed(1)}%`),(e=document.getElementById("non-losing-rate"))&&(e.textContent=68.15.toFixed(1)+"%"),(e=document.getElementById("total-bets"))&&(e.textContent=this.formatNumber(t)),(e=document.querySelector("#win-rate")?.parentElement?.querySelector(".kpi-subtitle"))&&(e.textContent="expected win rate"),(t=document.querySelector("#roi-performance")?.parentElement?.querySelector(".kpi-subtitle"))&&(t.textContent="projected return"),(e=document.querySelector("#non-losing-rate")?.parentElement?.querySelector(".kpi-subtitle"))&&(e.textContent="model accuracy"),(t=document.querySelector("#total-bets")?.parentElement?.querySelector(".kpi-subtitle"))&&(t.textContent="active recommendations"),this.updateMarketingTrend("win-rate-trend",64.91,"win"),this.updateMarketingTrend("roi-trend",23.81,"roi"),this.updateMarketingTrend("non-losing-trend",68.15,"nonlosing"),this.updateTrendIndicator("bets-trend",this.data.metrics&&this.data.metrics.bets_30d||0))}updateMarketingTrend(e,t,a){e=document.getElementById(e);e&&("win"===a&&60<=t?(e.textContent="🔥 Elite",e.className="kpi-trend positive"):"win"===a&&55<=t?(e.textContent="📈 Strong",e.className="kpi-trend positive"):"roi"===a&&15<=t?(e.textContent="💎 Premium",e.className="kpi-trend positive"):"roi"===a&&10<=t?(e.textContent="📊 Solid",e.className="kpi-trend positive"):"nonlosing"===a&&65<=t?(e.textContent="🛡️ Safe",e.className="kpi-trend positive"):0<t?(e.textContent="✅ Positive",e.className="kpi-trend positive"):(e.textContent="📊 Tracking",e.className="kpi-trend"))}updateTrendIndicator(e,t){e=document.getElementById(e);e&&(0<t?(e.textContent=`▲ ${Math.abs(t).toFixed(1)}%`,e.className="kpi-trend positive"):t<0?(e.textContent=`▼ ${Math.abs(t).toFixed(1)}%`,e.className="kpi-trend negative"):(e.textContent="━ 0.0%",e.className="kpi-trend"))}updateLastRunStatus(){var e=this.data.metrics.finished_at||this.data.metrics.started_at;e&&(document.getElementById("last-update").textContent="Last run: "+this.formatTimeAgo(e))}updateParlayWins(){var e=this.data.parlayWins||[];let t=new Date;t.setDate(t.getDate()-14);var a,e=e.filter(e=>((e.endDate instanceof Date&&!isNaN(e.endDate)?e.endDate:(e=(e.dateRange||"").split(" - "),(e=this.parseDateTimeSafe(e[e.length-1]))&&!isNaN(e)?e:null))||new Date)>=t),i=(document.getElementById("total-parlays").textContent=e.length,0<e.length?Math.max(...e.map(e=>e.returnPercent)):0);document.getElementById("max-payout").textContent=i.toFixed(0)+"%";let n=document.getElementById("parlay-grid");n.innerHTML="",0===e.length?n.innerHTML='<div style="grid-column: 1/-1; text-align: center; color: var(--muted); padding: var(--space-2xl);">No winning parlays found in recent data</div>':(i=window.innerWidth<=768?4:6,a=this.parlayPage*i,e.slice(a,a+i).forEach(e=>{var t=document.createElement("div");t.className="parlay-item",t.innerHTML=`
            ...
            `,a.appendChild(t)}))}getFilteredRecommendations(){let e=[...this.data.recommendations],t=new Date;if(e=e.filter(e=>e.datetime>t),"all"!==this.filters.league&&(e=e.filter(e=>e.league===this.filters.league)),"all"!==this.filters.confidence&&(e=e.filter(e=>e.confidence===this.filters.confidence)),0<this.filters.minEV&&(e=e.filter(e=>e.ev>=this.filters.minEV)),"all"!==this.filters.dateRange){var a=parseInt(this.filters.dateRange);let t=new Date;t.setDate(t.getDate()-a),e=e.filter(e=>e.datetime>=t)}return e}exportFilteredRecommendations(){var e=this.getFilteredRecommendations(),e=this.arrayToCSV(e,["datetime","league","home","away","recommendation","line","odds","ev","confidence"]);this.downloadCSV(e,"parlayking_recommendations.csv")}arrayToCSV(e,a){let i=[];return i.push(a.join(",")),e.forEach(t=>{var e=a.map(e=>{e=t[e];return"string"==typeof e?`"${e}"`:e});i.push(e.join(","))}),i.join("\n")}downloadCSV(e,t){var e=new Blob([e],{type:"text/csv"}),e=window.URL.createObjectURL(e),a=document.createElement("a");a.href=e,a.download=t,a.click(),window.URL.revokeObjectURL(e)}getFilteredGames(){let e=[...this.data.unifiedGames||[]],i=new Date;if("today"===this.filters.gameTimeFilter){let t=new Date(i);t.setHours(23,59,59,999),e=e.filter(e=>e.datetime>=i&&e.datetime<=t)}else if("tomorrow"===this.filters.gameTimeFilter){let t=new Date(i),a=(t.setDate(t.getDate()+1),t.setHours(0,0,0,0),new Date(t));a.setHours(23,59,59,999),e=e.filter(e=>e.datetime>=t&&e.datetime<=a)}else if("weekend"===this.filters.gameTimeFilter){let t=new Date(i),a=(t.setDate(i.getDate()+(5-i.getDay()+7)%7),t.setHours(18,0,0,0),new Date(t));a.setDate(t.getDate()+2),a.setHours(23,59,59,999),e=e.filter(e=>e.datetime>=t&&e.datetime<=a)}else"recommendations"===this.filters.gameTimeFilter&&(e=e.filter(e=>e.hasRecommendation));return(e="all"!==this.filters.league?e.filter(e=>e.league===this.filters.league):e).sort((e,t)=>e.tier!==t.tier?e.tier-t.tier:e.datetime-t.datetime)}renderUnifiedSchedule(){var e,t=document.getElementById("unified-games-container");t?this.data.unifiedGames&&0!==this.data.unifiedGames.length?0===(e=this.getFilteredGames()).length?t.innerHTML='<div class="no-games">No games found for the selected filters.</div>':(e=this.groupGamesByTime(e),t.innerHTML=Object.entries(e).map(([e,t])=>this.renderTimeGroup(e,t)).join(""),this.initGameCardInteractions()):this.renderFallbackRecommendations(t):console.log("Unified games container not found - skipping schedule render")}renderFallbackRecommendations(e){var t=this.getFilteredRecommendations().slice(0,10);0===t.length?e.innerHTML='<div class="no-games">No upcoming recommendations available.</div>':(e.innerHTML=`
        ...
        `}groupGamesByTime(e){var t=new Date;let a=new Date(t.getFullYear(),t.getMonth(),t.getDate()),i=new Date(a),n=(i.setDate(i.getDate()+1),new Date(i)),r=(n.setDate(n.getDate()+1),{Today:[],Tomorrow:[],"This Weekend":[],"Next Week":[]});return e.forEach(e=>{(e.datetime>=a&&e.datetime<i?r.Today:e.datetime>=i&&e.datetime<n?r.Tomorrow:5<=e.datetime.getDay()||e.datetime.getDay()<=1?r["This Weekend"]:r["Next Week"]).push(e)}),Object.keys(r).forEach(e=>{0===r[e].length&&delete r[e]}),r}renderTimeGroup(e,t){return`
            <div class="time-group">
                <div class="time-group-header">
                    <h4>${e}</h4>
        ...
        `}renderGameCard(e,t){var a=this.getSignalIcon(e.primarySignal),i=e.hasRecommendation,n=this.formatGameTime(e.datetime),r=this.formatOddsCompact(e);return`
            <div class="game-card" 
                 data-game-index="${t}"
                ...
                <div class="game-row">
                    <div class="game-time">${n}</div>
                    <div class="signal-container">
                        ${a?`<span class="signal-icon ${e.primarySignal}" title="${this.getSignalTitle(e.primarySignal)}">${a}</span>`:""}
                        ${e.secondarySignal?'<span class="signal-dot"></span>':""}
                    </div>
                    <div class="game-teams">
                        <span class="league-flag">${e.leagueFlag}</span>
                        <div class="teams-text">
                            <span class="home">${e.home}</span>
                            <span class="vs">vs</span>
                            <span class="away">${e.away}</span>
                    ...
                    <div class="game-league mobile-hidden">${e.leagueShort}</div>
                    <div class="game-odds mobile-hidden">${r}</div>
                    <div class="expand-btn">${i?"▼":""}</div>
                </div>
                
                ${i?this.renderExpandedContent(e):""}
            </div>
        `}getSignalIcon(e){return{"kings-call":"👑","high-ev":"📈","hot-pick":"🔥","value-bet":"💎"}[e]||""}getSignalTitle(e){return{"kings-call":"King's Call Available","high-ev":"High Expected Value","hot-pick":"Hot Pick - High Confidence","value-bet":"Value Bet Opportunity"}[e]||""}formatGameTime(e){return e?e.toLocaleTimeString("en-GB",{hour:"2-digit",minute:"2-digit",hour12:!1}):"--:--"}formatOddsCompact(e){return e.odds1&&e.oddsX&&e.odds2?`${e.odds1.toFixed(2)} ${e.oddsX.toFixed(2)} `+e.odds2.toFixed(2):"TBD"}renderExpandedContent(e){return`
            <div class="expanded-details hidden">
                <div class="expanded-row recommendation-row">
                    <strong>Our Pick:</strong> ${e.recText}
                    <span class="ev-badge">EV: ${(100*e.ev).toFixed(1)}%</span>
                </div>
                
                ${e.kingsCall&&!e.kingsCall.includes("Unable to fetch")?`
                    <div class="expanded-row kings-call-row">
                        <strong>King's Call:</strong> 
                        <span class="kings-call-text">${e.kingsCall}</span>
                    </div>
                ...
                <div class="expanded-row stats-row">
                    ...
                    <span class="mobile-visible">Odds: ${this.formatOddsCompact(e)}</span>
                </div>
                
                <div class="expanded-actions">
                    <button class="action-btn secondary" onclick="parlayKing.shareGame(${e.datetime.getTime()})">
                        Share Pick
                    </button>
                    <button class="action-btn primary" onclick="parlayKing.exportGame(${e.datetime.getTime()})">
                        Export
                    </button>
                </div>
            </div>
        `}initGameCardInteractions(){setTimeout(()=>{document.querySelectorAll('.game-card[data-expandable="true"]').forEach(t=>{t.addEventListener("click",e=>{e.target.closest(".expanded-actions")||e.target.closest(".action-btn")||this.toggleGameExpansion(t)})})},100)}toggleGameExpansion(e){var t=e.querySelector(".expanded-details"),a=e.querySelector(".expand-btn");t&&a&&(t.classList.contains("hidden")?(t.classList.remove("hidden"),a.textContent="▲",e.classList.add("expanded"),this.trackEvent&&this.trackEvent("game_expanded",{signal:e.dataset.signal,league:e.querySelector(".game-league")?.textContent})):(t.classList.add("hidden"),a.textContent="▼",e.classList.remove("expanded")))}shareGame(t){var e=this.data.unifiedGames.find(e=>e.datetime.getTime()===t);e&&(e=e.hasRecommendation?`🎯 ${e.recText} @ ${e.recOdds.toFixed(2)} odds (EV: ${(100*e.ev).toFixed(1)}%) - ${e.home} vs `+e.away:`⚽ ${e.home} vs ${e.away} - `+this.formatDateTime(e.datetime),navigator.share?navigator.share({title:"ParlayKing Betting Pick",text:e,url:window.location.href}):navigator.clipboard.writeText(e).then(()=>{alert("Shared text copied to clipboard!")}))}exportGame(t){var e,a=this.data.unifiedGames.find(e=>e.datetime.getTime()===t);a&&a.hasRecommendation&&(e=[["Date/Time","League","Match","Recommendation","Odds","EV (%)","Confidence","King's Call"],[this.formatDateTime(a.datetime),a.league,a.home+" vs "+a.away,a.recText,a.recOdds.toFixed(2),(100*a.ev).toFixed(1)+"%",a.confidence,a.kingsCall]].map(e=>e.join(",")).join("\n"),this.downloadCSV(e,`pick_${a.home.replace(/\s+/g,"")}_vs_${a.away.replace(/\s+/g,"")}.csv`))}formatNumber(e){return(new Intl.NumberFormat).format(e)}formatCurrency(e,t=!1){t=t&&0<=e?"+":"";return 1e3<=Math.abs(e)?t+(e/1e3).toFixed(1)+"k":t+e.toFixed(2)}formatDate(e){e=e instanceof Date?e:this.parseDateTimeSafe(e);return e?new Intl.DateTimeFormat("en-US",{month:"short",day:"numeric",timeZone:"Asia/Singapore"}).format(e):"--"}formatDateTime(e){e=e instanceof Date?e:this.parseDateTimeSafe(e);return e?new Intl.DateTimeFormat("en-US",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0,timeZone:"Asia/Singapore"}).format(e):"--"}formatTimeAgo(e){var t;return e?(e=new Date(e),t=new Date,(t=Math.floor((t-e)/36e5))<1?"Just now":t<24?t+"h ago":Math.floor(t/24)+"d ago"):"--"}showLoading(e){var t=document.getElementById("loading-overlay");e?t.classList.remove("hidden"):t.classList.add("hidden")}showError(e){console.error(e)}initAnalyticsPage(){this.showLoading(!0),this.loadAllData().then(()=>{var e=document.getElementById("tier-select"),e=(e&&e.addEventListener("change",()=>this.renderROIHeatmap()),document.querySelectorAll(".range-btn[data-min-bets]").forEach(e=>{e.addEventListener("click",e=>{document.querySelectorAll(".range-btn[data-min-bets]").forEach(e=>e.classList.remove("active")),e.currentTarget.classList.add("active"),this.renderROIHeatmap()})}),document.getElementById("segments-tier-select"));e&&e.addEventListener("change",()=>this.renderTopSegments()),document.querySelectorAll(".toggle-btn[data-view]").forEach(e=>{e.addEventListener("click",e=>{var t=e.currentTarget.parentElement;t&&(t.querySelectorAll(".toggle-btn").forEach(e=>e.classList.remove("active")),e.currentTarget.classList.add("active"),this.renderPnLChart())})}),this.renderROIHeatmap(),this.renderPnLChart(),this.renderTopSegments(),this.updateLastRunStatus(),this.showLoading(!1)}).catch(e=>{console.error("Failed to load analytics data:",e),this.showError("Failed to load analytics data"),this.showLoading(!1)})}renderROIHeatmap(){var e=document.getElementById("roi-heatmap");if(e){let t=parseInt(document.querySelector(".range-btn.active[data-min-bets]")?.dataset.minBets||"30",10),s=parseInt(document.getElementById("tier-select")?.value||"1",10);let l=(Array.isArray(this.data.roiHeatmap)&&0<this.data.roiHeatmap.length?this.data.roiHeatmap.map(e=>({tier:+e.tier,line:+e.line,roi_pct:+e.roi_pct,n:+e.n})):this.getBacktestROIHeatmapFallback()).filter(e=>e.tier===s&&e.n>=t);var a=[...new Set(l.map(e=>e.line))].sort((e,t)=>e-t);if(0===a.length)e.innerHTML='<div class="chart-placeholder">No segments meet the selected filters</div>';else if(a.length<4){let r='<div class="segments-pills">';a.forEach(t=>{var e=l.find(e=>e.line===t)||{roi_pct:0,n:0},a=e.roi_pct,i=this.classForRoi(a),n=this.createSparklineFromRoi(a,`hm-${s}-`+t);r+=`
                    ...
                    </div>`}),r+="</div>",void(e.innerHTML=r)}else{let n=`<div class="heatmap-grid" style="grid-template-columns: repeat(${Math.max(a.length,1)}, 1fr);">`;a.forEach(t=>{var e=l.find(e=>e.line===t)||{roi_pct:0,n:0},a=e.roi_pct,i=10<=a?"heat-pos":3<=a?"heat-mid":"heat-neg";n+=`
                ...
                </div>`}),n+="</div>",e.innerHTML=n}}}getBacktestROIHeatmapFallback(){return[{tier:1,line:-.5,roi:28.4,bets:67},{tier:1,line:-.25,roi:25.1,bets:54},{tier:1,line:0,roi:22.7,bets:89},{tier:1,line:.25,roi:19.8,bets:43},{tier:1,line:.5,roi:24.3,bets:31},{tier:2,line:-.5,roi:21.6,bets:39},{tier:2,line:-.25,roi:18.9,bets:45},{tier:2,line:0,roi:20.4,bets:67},{tier:2,line:.25,roi:17.2,bets:38},{tier:3,line:0,roi:15.8,bets:20}].map(e=>({tier:e.tier,line:e.line,roi_pct:e.roi,n:e.bets}))}renderPnLChart(){var c=document.getElementById("pnl-chart");if(c){let e=Array.isArray(this.data.pnlByMonth)&&0<this.data.pnlByMonth.length?this.data.pnlByMonth:[{month:"2024-03",league:"England Premier League",pnl:1247.32},{month:"2024-03",league:"Spain La Liga",pnl:891.45},{month:"2024-04",league:"England Premier League",pnl:1534.67},{month:"2024-04",league:"Germany Bundesliga",pnl:1123.89},{month:"2024-04",league:"Italy Serie A",pnl:678.23},{month:"2024-05",league:"England Premier League",pnl:1789.34},{month:"2024-05",league:"Spain La Liga",pnl:1345.78},{month:"2024-05",league:"France Ligue 1",pnl:867.89}];var u=[...new Set(e.map(e=>String(e.month)))].sort();let i=[...new Set(e.map(e=>String(e.league)))].map(t=>({league:t,total:e.filter(e=>e.league===t).reduce((e,t)=>e+(parseFloat(t.pnl)||0),0)})).sort((e,t)=>Math.abs(t.total)-Math.abs(e.total)).slice(0,6).map(e=>e.league);var v=u.map(t=>{let a={};return i.forEach(e=>a[e]=0),e.filter(e=>String(e.month)===t&&i.includes(String(e.league))).forEach(e=>{a[String(e.league)]+=parseFloat(e.pnl)||0}),{month:t,byLeague:a}});let n=0,r=0,a=(v.forEach(e=>{let t=0,a=0;Object.values(e.byLeague).forEach(e=>{0<=e?t+=e:a+=e}),r=Math.max(r,t),n=Math.min(n,a)}),0===r&&0===n&&(r=1),c.innerHTML="",document.createElementNS("http://www.w3.org/2000/svg","svg")),s=(a.setAttribute("class","chart-svg"),a.setAttribute("viewBox","0 0 800 400"),a.setAttribute("preserveAspectRatio","xMidYMid meet"),{top:20,right:24,bottom:48,left:64});var f=800-s.left-s.right;let l=400-s.top-s.bottom,m=f/Math.max(u.length,1),h=e=>s.top+l-(e-n)/(r-n)*l;var y=document.createElementNS("http://www.w3.org/2000/svg","g");for(let e=0;e<=5;e++){var w=s.top+e/5*l,b=document.createElementNS("http://www.w3.org/2000/svg","line");b.setAttribute("x1",s.left),b.setAttribute("y1",w),b.setAttribute("x2",s.left+f),b.setAttribute("y2",w),b.setAttribute("class","chart-grid-line"),y.appendChild(b)}a.appendChild(y);let t=["#FF7A45","#3A7BD5","#2ECC71","#F39C12","#9B59B6","#00C2A8"],g=e=>t[i.indexOf(e)%t.length],d=document.querySelector(".toggle-btn.active[data-view]")?.dataset.view||"stacked",p=document.createElementNS("http://www.w3.org/2000/svg","g");v.forEach((o,e)=>{let c=s.left+e*m;if("separated"===d){let r=Math.max(6,(m-16)/Math.max(i.length,1));i.forEach((e,t)=>{var a=o.byLeague[e]||0,t=c+8+t*r,i=Math.min(h(0),h(a)),a=Math.abs(h(a)-h(0)),n=document.createElementNS("http://www.w3.org/2000/svg","rect");n.setAttribute("x",t.toFixed(1)),n.setAttribute("y",i.toFixed(1)),n.setAttribute("width",Math.max(4,r-2).toFixed(1)),n.setAttribute("height",Math.max(0,a).toFixed(1)),n.setAttribute("fill",g(e)),n.setAttribute("rx","3"),p.appendChild(n)})}else{let s=Math.max(10,m-16),l=0,d=0;i.forEach(e=>{var t,a,i,n,r=o.byLeague[e]||0;0!==r&&(t=(a=0<=r?l:d)+r,a=h(a),i=h(t),(n=document.createElementNS("http://www.w3.org/2000/svg","rect")).setAttribute("x",(c+(m-s)/2).toFixed(1)),n.setAttribute("y",Math.min(a,i).toFixed(1)),n.setAttribute("width",s.toFixed(1)),n.setAttribute("height",Math.abs(i-a).toFixed(1)),n.setAttribute("fill",g(e)),n.setAttribute("rx","4"),p.appendChild(n),0<=r?l=t:d=t)})}var e=document.createElementNS("http://www.w3.org/2000/svg","text"),t=(e.setAttribute("x",(c+m/2).toFixed(1)),e.setAttribute("y",(s.top+l+24).toFixed(1)),e.setAttribute("text-anchor","middle"),e.setAttribute("class","chart-axis-text"),new Date(o.month+"-01"));e.textContent=t.toLocaleString("en-US",{month:"short"}),a.appendChild(e)}),a.appendChild(p);var x=document.createElementNS("http://www.w3.org/2000/svg","g");for(let e=0;e<=5;e++){var E=n+(r-n)*(e/5),C=s.top+l-e/5*l+4,S=document.createElementNS("http://www.w3.org/2000/svg","text");S.setAttribute("x",s.left-10),S.setAttribute("y",C),S.setAttribute("text-anchor","end"),S.setAttribute("class","chart-axis-text"),S.textContent=this.formatCurrency(E,!0),x.appendChild(S)}a.appendChild(x),c.appendChild(a);let o=document.createElement("div");o.className="chart-legend",i.forEach(e=>{var t=document.createElement("div");t.className="legend-item",t.innerHTML=`<span class="legend-swatch" style="background:${g(e)}"></span><span class="legend-label">${e}</span>`,o.appendChild(t)}),c.appendChild(o)}}createBacktestPnLChart(e){var t=[{month:"2024-03",pnl:1247.32,league:"England Premier League"},{month:"2024-03",pnl:891.45,league:"Spain La Liga"},{month:"2024-04",pnl:1534.67,league:"England Premier League"},{month:"2024-04",pnl:1123.89,league:"Germany Bundesliga"},{month:"2024-04",pnl:678.23,league:"Italy Serie A"},{month:"2024-05",pnl:1789.34,league:"England Premier League"},{month:"2024-05",pnl:1345.78,league:"Spain La Liga"},{month:"2024-05",pnl:867.89,league:"France Ligue 1"}],a=t.reduce((e,t)=>(e[t.month]||(e[t.month]=0),e[t.month]+=t.pnl,e),{}),a=Object.entries(a).map(([e,t])=>({date:new Date(e+"-01"),value:t})).sort((e,t)=>e.date-t.date);let i=0,n=a.map(e=>(i+=e.value,{date:e.date,value:i}));e.innerHTML='<div id="backtest-pnl-chart" class="custom-chart" style="height: 300px;"></div>',setTimeout(()=>{this.createCustomChart("backtest-pnl-chart",n,{formatY:e=>this.formatCurrency(e,!0)})},100),e.innerHTML+=`
        ...
        `}seededRandom(t){let a=2166136261;for(let e=0;e<t.length;e++)a^=t.charCodeAt(e),a=Math.imul(a,16777619);return()=>{a+=1831565813;var e=Math.imul(a^a>>>15,1|a);return(((e^=e+Math.imul(e^e>>>7,61|e))^e>>>14)>>>0)/4294967296}}createSparklineFromRoi(n,e){let r=this.seededRandom(String(e));e=Array.from({length:8},(e,t)=>{var a=.45+n/100*.3,i=.2*(r()-.5);return Math.max(.1,Math.min(.9,a+i))});let a=76/(e.length-1);var t=e.map((e,t)=>[2+t*a,22-20*e]);let i=`M ${t[0][0]} `+t[0][1];for(let e=1;e<t.length;e++)i+=` L ${t[e][0]} `+t[e][1];return`<svg class="spark-svg" width="80" height="24" viewBox="0 0 80 24" preserveAspectRatio="none" aria-hidden="true">
      <path d="${""+i+` L ${t[t.length-1][0]} 22 L ${t[0][0]} 22 Z`}" class="spark-area"></path>
      <path d="${i}" class="spark-line"></path>
    </svg>`}classForRoi(e){return 10<=e?"excellent":3<=e?"good":"moderate"}renderTopSegments(){let s=document.getElementById("segments-tbody");if(s){s.innerHTML="";let t=parseInt(document.getElementById("segments-tier-select")?.value||"1",10);this.data.topSegments&&0!==this.data.topSegments.length?this.data.topSegments.filter(e=>parseInt(e.tier)===t).sort((e,t)=>parseFloat(t.roi_pct)-parseFloat(e.roi_pct)).forEach(e=>{var t=document.createElement("tr"),a=parseFloat(e.roi_pct),i=parseFloat(e.line),n=this.classForRoi(a),r=this.createSparklineFromRoi(a,e.tier+"-"+i);t.innerHTML=`
                    <td>Tier ${e.tier}</td>
