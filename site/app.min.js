class ParlayKing{constructor(){this.cache=new Map,this.data={metrics:{},pnlByMonth:[],bankrollSeries:[],recommendations:[],roiHeatmap:[],topSegments:[],settledBets:[],parlayWins:[]},this.filters=this.getFiltersFromURL(),this.version=window.__PK_VERSION||"latest",this.parlayPage=0,this.featureFlags={mobile_card_v2:"true"===localStorage.getItem("mobile_card_v2")},this.mobileState={filtersDrawerOpen:!1,renderedGames:20,isInfiniteScrollEnabled:this.featureFlags.mobile_card_v2},this.init()}parseParlayWins(e){return Array.isArray(e)&&0!==e.length?e.map(e=>{var t=parseFloat(e.stake||100),a=parseFloat(e.total_odds||e.odds||1),i=parseFloat(e.payout||t*a),n=parseFloat(e.profit||i-t),s=(e.legs||e.legs_str||"").toString().split(" || ").filter(Boolean).map(e=>{var[e,t]=e.split(" | "),[e,a]=(e||"").split(" vs ").map(e=>(e||"").trim());let i=t||"",n=1;t=i.lastIndexOf("@");return-1!==t&&(n=parseFloat(i.slice(t+1))||1,i=i.slice(0,t).trim()),{home:e,away:a,recommendation:i,odds:n}}),r=this.parseDateTimeSafe(e.window_start||e.start_dt||e.start),l=this.parseDateTimeSafe(e.window_end||e.end_dt||e.end);return{legs:s,legCount:parseInt(e.leg_count||s.length||0),totalOdds:a,stake:t,totalPayout:i,profit:n,returnPercent:n/Math.max(t,1)*100,dateRange:r&&l?this.isSameDay(r,l)?this.formatDate(r):this.formatDate(r)+" - "+this.formatDate(l):"--",startDate:r||null,endDate:l||null}}):[]}async init(){this.showLoading(!0),await this.loadAllData(),this.initializeUI(),this.updateUI(),this.showLoading(!1)}async loadCSV(t,a=!0){window.Papa||await new Promise(e=>{var t=document.createElement("script");t.src="https://unpkg.com/papaparse@5.4.1/papaparse.min.js",t.onload=e,document.head.appendChild(t)});var i="csv_"+t;if(a&&this.cache.has(i))return this.cache.get(i);try{var n,s,r,l=this.cache.has(i),d=a&&l?{"If-None-Match":localStorage.getItem("etag_"+t)||""}:{};let e=await fetch(t+"?t="+Date.now(),{headers:d});if((e=304!==e.status||l?e:await fetch(t+"?t="+Date.now(),{cache:"no-store"})).ok)return n=await e.text(),s=Papa.parse(n,{header:!0,dynamicTyping:!0,skipEmptyLines:!0}).data,this.cache.set(i,s),(r=e.headers.get("ETag"))&&localStorage.setItem("etag_"+t,r),s;throw new Error(`Failed to load ${t}: `+e.status)}catch(e){return console.warn(`Failed to load ${t}:`,e),this.cache.get(i)||[]}}async loadAllData(){try{var[e,t,a,i,n,s,r,l,d]=await Promise.all([this.loadCSV("metrics.csv"),this.loadCSV("pnl_by_month.csv"),this.loadCSV("bankroll_series_90d.csv"),this.loadCSV("latest_recommendations.csv"),this.loadCSV("roi_heatmap.csv"),this.loadCSV("top_segments.csv"),this.loadCSV("settled_bets.csv"),this.loadCSV("parlay_wins.csv").catch(()=>[]),this.loadCSV("unified_games.csv").catch(e=>(console.warn("Unified games CSV not available:",e),[]))]),o=(this.data={metrics:this.parseMetrics(e),pnlByMonth:t,bankrollSeries:this.parseBankrollSeries(a),recommendations:this.parseRecommendations(i),roiHeatmap:n,topSegments:s,settledBets:this.parseSettledBets(r),unifiedGames:this.parseUnifiedGames(d)},this.parseParlayWins(l||[]));this.data.parlayWins=o&&0<o.length?o:this.calculateParlayWinsFromBets(),this.populateFilterOptions(),document.querySelectorAll(".lazy-load").forEach(e=>{e.classList.add("loaded")})}catch(e){console.error("Failed to load data:",e),this.showError("Failed to load dashboard data. Please try refreshing the page.")}}parseMetrics(e){let t={};return Array.isArray(e)&&e.forEach(e=>{e&&e.metric&&null!=e.value&&""!==e.value&&(t[e.metric]=e.value)}),t}parseDateTimeSafe(e){if(!e)return null;if(e instanceof Date)return isNaN(e)?null:e;try{var t,a,i,n,s,r,l,d,o,c=String(e).replace(" ","T"),m=new Date(c);return isNaN(m)?(m=new Date(c+"Z"),isNaN(m)?([t,a]=String(e).split(" "),[i,n,s]=t.split("-").map(Number),[r=0,l=0,d=0]=(a||"").split(":").map(Number),o=new Date(i,(n||1)-1,s||1,r,l,d),isNaN(o)?null:o):m):m}catch(e){return null}}parseGmt8(t){if(!t)return null;try{var[e,a="00:00:00"]=String(t).split(" "),[i,n,s]=e.split("-").map(Number),[r,l,d]=a.split(":").map(Number),o=Date.UTC(i,(n||1)-1,s||1,(r||0)-8,l||0,d||0);return new Date(o)}catch(e){return this.parseDateTimeSafe(t)}}parseBankrollSeries(e){return Array.isArray(e)?e.filter(e=>e&&e.dt_gmt8&&void 0!==e.cum_bankroll).map(e=>({date:this.parseDateTimeSafe(e.dt_gmt8),bankroll:parseFloat(e.cum_bankroll)||0})).sort((e,t)=>e.date-t.date):[]}parseRecommendations(e){return Array.isArray(e)?e.filter(e=>e&&e.dt_gmt8&&e.home&&e.away).map(e=>{var t=this.parseGmt8(e.dt_gmt8)||this.parseDateTimeSafe(e.dt_gmt8);return t?{datetime:t,league:e.league||"",home:e.home,away:e.away,recommendation:e.rec_text||e.recommendation||"",line:parseFloat(e.line)||0,odds:parseFloat(e.odds)||0,ev:parseFloat(e.ev)||0,confidence:e.confidence||"Medium",kingsCall:e.kings_call_insight||"No additional insights available."}:(console.warn("Invalid date for recommendation: "+e.dt_gmt8),null)}).filter(Boolean).sort((e,t)=>{var a=e.datetime-t.datetime;return console.log(`Sorting: ${e.home} vs ${e.away} (${e.datetime}) vs ${t.home} vs ${t.away} (${t.datetime}) = `+a),a}):[]}parseSettledBets(e){return Array.isArray(e)?e.filter(e=>e&&e.home&&e.away).map(e=>{var t=(e.bet_type_refined_ah||e.bet_type||"").toString(),a=parseFloat(e.line_betted_on_refined||e.line||0),i=parseFloat(e.odds_betted_on_refined||e.odds||0),n=parseFloat(e.pl||0),s=(e.status||"").toString().toLowerCase(),s=0<n||"won"===s||"win"===s,r=0===n,l=!r&&i||1,d=t.toLowerCase();let o;return o=d.includes("away")?e.away+" "+(0<=a?"+":"")+(isFinite(a)?a.toFixed(2):"0.00"):d.includes("home")?e.home+" "+(0<=a?"+":"")+(isFinite(a)?a.toFixed(2):"0.00"):t?t+" "+(0<=a?"+":"")+(isFinite(a)?a.toFixed(2):"0.00"):"AH "+(0<=a?"+":"")+(isFinite(a)?a.toFixed(2):"0.00"),{fixture_id:e.fixture_id,league:e.league||"",home:e.home,away:e.away,home_score:parseInt(e.home_score||0),away_score:parseInt(e.away_score||0),bet_type:t,line:a,odds:i||1,effectiveOdds:l,stake:parseFloat(e.stake||0),pl:n,isWin:s,isPush:r,recommendation:o,status:e.status||"",datetime:this.parseGmt8(e.dt_gmt8)||this.parseDateTimeSafe(e.dt_gmt8)}}).sort((e,t)=>(t.datetime||0)-(e.datetime||0)):[]}parseUnifiedGames(e){return Array.isArray(e)&&0!==e.length?e.filter(e=>e&&e.datetime_gmt8&&e.home_name&&e.away_name).map(e=>{var t=this.parseGmt8(e.datetime_gmt8)||this.parseDateTimeSafe(e.datetime_gmt8);return t?{datetime:t,league:e.league||"",leagueShort:e.league_short||e.league||"",leagueFlag:e.league_flag||"⚽",leagueColor:e.league_color||"#666666",home:e.home_name,away:e.away_name,odds1:parseFloat(e.odds_1)||0,oddsX:parseFloat(e.odds_x)||0,odds2:parseFloat(e.odds_2)||0,tier:parseInt(e.league_tier)||3,competitionType:e.competition_type||"league",isFuture:"True"===e.is_future||!0===e.is_future||"true"===e.is_future,status:e.status||"",hasRecommendation:"True"===e.has_recommendation||!0===e.has_recommendation||"true"===e.has_recommendation,recText:e.rec_text||"",line:parseFloat(e.line)||0,recOdds:parseFloat(e.rec_odds)||0,ev:parseFloat(e.ev)||0,confidence:e.confidence||"",kingsCall:e.kings_call||"",kingsCallAgreement:e.kings_call_agreement||"",primarySignal:e.primary_signal||"",secondarySignal:e.secondary_signal||""}:(console.warn("Invalid date for game: "+e.datetime_gmt8),null)}).filter(Boolean).map(e=>{var t=this.parseAsianHandicapData(e);return{...e,...t}}).sort((e,t)=>e.datetime-t.datetime):(console.log("No unified games data available - will use fallback recommendations"),[])}parseAsianHandicapData(e){let t=null,a=null,i=null,n=!1;var s,r;return void 0!==e.ah_line_home&&void 0!==e.ah_line_away?(a=parseFloat(e.ah_line_home),i=parseFloat(e.ah_line_away),e.hasRecommendation&&e.recText&&(s=e.recText.match(/^(.+?)\s+([-+]?\d*\.?\d+)$/))&&(r=s[1].trim(),s=parseFloat(s[2]),r===e.home&&Math.abs(s-a)<.01?t=e.home:r===e.away&&Math.abs(s-i)<.01?t=e.away:(e.home.includes(r)||r.includes(e.home))&&Math.abs(s-a)<.01?t=e.home:(e.away.includes(r)||r.includes(e.away))&&Math.abs(s-i)<.01&&(t=e.away))):e.hasRecommendation&&e.recText?(r=e.recText.match(/^(.+?)\s+([-+]?\d*\.?\d+)$/))&&(t=r[1].trim(),s=parseFloat(r[2]),t===e.home?(a=s,i=-s):t===e.away?(i=s,a=-s):e.home.includes(t)||t.includes(e.home)?(a=s,i=-s,t=e.home):(e.away.includes(t)||t.includes(e.away))&&(i=s,a=-s,t=e.away)):0<e.odds1&&0<e.oddsX&&0<e.odds2&&(console.warn("Using 1X2 estimation for AH lines - CSV data should be authoritative"),e=2.5*((r=1/e.odds1)/(r=r+1/e.oddsX+(s=1/e.odds2))-s/r),a=1.875<e?-2:1.625<e?-1.75:1.375<e?-1.5:1.125<e?-1.25:.875<e?-1:.625<e?-.75:.375<e?-.5:.125<e?-.25:-.125<e?0:-.375<e?.25:-.625<e?.5:-.875<e?.75:-1.125<e?1:-1.375<e?1.25:-1.625<e?1.5:-1.875<e?1.75:2,i=-a),null!==a&&null!==i&&(n=0===a&&0===i,a=Math.round(4*a)/4,i=Math.round(4*i)/4),{recommendedTeam:t,homeLine:a,awayLine:i,isPk:n,hasAhData:null!==a&&null!==i}}calculateParlayWinsFromBets(){var e=this.data.settledBets||[],e=this.groupBetsByWindow(e,12);let a=[];return e.forEach(e=>{var t=e.filter(e=>e.isWin||e.isPush);for(let e=3;e<=Math.min(6,t.length);e++)this.getCombinations(t,e).sort((e,t)=>this.calculateParlayOdds(t)-this.calculateParlayOdds(e)).slice(0,2).forEach(e=>a.push(this.createParlayItem(e)))}),a.sort((e,t)=>t.totalPayout-e.totalPayout).slice(0,6)}groupBetsByWindow(e,t=0){let a=new Map,s=(e,t,a,i)=>new Date(Date.UTC(e,t-1,a,i-8,0,0)),i=e=>{var e=new Date(e.getTime()+288e5),t=e.getUTCFullYear(),a=e.getUTCMonth()+1,i=e.getUTCDate(),n=e.getUTCHours();return 5<=n&&n<17?s(t,a,i,5):17<=n?s(t,a,i,17):(n=new Date(e.getTime()-864e5),s(n.getUTCFullYear(),n.getUTCMonth()+1,n.getUTCDate(),17))};return e.forEach(e=>{var t;e.datetime&&(t=i(e.datetime).getTime(),a.has(t)||a.set(t,[]),a.get(t).push(e))}),Array.from(a.values())}getCombinations(a,e){if(1===e)return a.map(e=>[e]);if(e>a.length)return[];let i=[];for(let t=0;t<a.length-e+1;t++)this.getCombinations(a.slice(t+1),e-1).forEach(e=>{i.push([a[t],...e])});return i}calculateParlayOdds(e){return e.reduce((e,t)=>e*(t.effectiveOdds||t.odds||1),1)}createParlayItem(e){var t=this.calculateParlayOdds(e),a=100*t,i=a-100;return{legs:e,legCount:e.length,totalOdds:t,stake:100,totalPayout:a,profit:i,returnPercent:i/100*100,dateRange:this.getParlayDateRange(e)}}getParlayDateRange(e){var e=e.map(e=>e.datetime).sort(),t=e[0],e=e[e.length-1];return this.isSameDay(t,e)?this.formatDate(t):this.formatDate(t)+" - "+this.formatDate(e)}isSameDay(e,t){return e.toDateString()===t.toDateString()}createCustomChart(g,p,u={}){let v=document.getElementById(g);if(v)if(v.innerHTML="",p&&Array.isArray(p)&&0!==p.length){let h=p.filter(e=>e&&"number"==typeof e.value&&!isNaN(e.value));if(0!==h.length){var g=document.createElementNS("http://www.w3.org/2000/svg","svg"),p=(g.setAttribute("class","chart-svg"),g.setAttribute("viewBox","0 0 800 400"),g.setAttribute("preserveAspectRatio","xMidYMid meet"),document.createElementNS("http://www.w3.org/2000/svg","defs")),f=document.createElementNS("http://www.w3.org/2000/svg","linearGradient"),y=(f.setAttribute("id","chartGradient"),f.setAttribute("x1","0%"),f.setAttribute("y1","0%"),f.setAttribute("x2","0%"),f.setAttribute("y2","100%"),document.createElementNS("http://www.w3.org/2000/svg","stop")),w=(y.setAttribute("offset","0%"),y.setAttribute("stop-color","#FF7A45"),y.setAttribute("stop-opacity","0.3"),document.createElementNS("http://www.w3.org/2000/svg","stop"));w.setAttribute("offset","100%"),w.setAttribute("stop-color","#FF7A45"),w.setAttribute("stop-opacity","0.05"),f.appendChild(y),f.appendChild(w),p.appendChild(f),g.appendChild(p);let a={top:20,right:30,bottom:40,left:60},i=800-a.left-a.right,n=400-a.top-a.bottom,s=e=>a.left+e/Math.max(h.length-1,1)*i,t=Math.min(...h.map(e=>e.value)),r=Math.max(...h.map(e=>e.value)),l=.1*(r-t),d=e=>a.top+n-(e-(t-l))/(r+l-(t-l))*n;var b=document.createElementNS("http://www.w3.org/2000/svg","g");for(let t=0;t<=5;t++){var x=a.top+t/5*n;let e=document.createElementNS("http://www.w3.org/2000/svg","line");e.setAttribute("x1",a.left),e.setAttribute("y1",x),e.setAttribute("x2",a.left+i),e.setAttribute("y2",x),e.setAttribute("class","chart-grid-line"),b.appendChild(e)}g.appendChild(b);let o=`M ${a.left} `+(a.top+n);h.forEach((e,t)=>{o+=` L ${s(t)} `+d(e.value)}),o+=` L ${a.left+i} ${a.top+n} Z`;y=document.createElementNS("http://www.w3.org/2000/svg","path");y.setAttribute("d",o),y.setAttribute("class","chart-area"),g.appendChild(y);let c=`M ${s(0)} `+d(h[0].value),e=(h.slice(1).forEach((e,t)=>{c+=` L ${s(t+1)} `+d(e.value)}),document.createElementNS("http://www.w3.org/2000/svg","path")),m=(e.setAttribute("d",c),e.setAttribute("class","chart-line"),g.appendChild(e),document.createElementNS("http://www.w3.org/2000/svg","g"));h.forEach((t,e)=>{var a=document.createElementNS("http://www.w3.org/2000/svg","circle");a.setAttribute("cx",s(e)),a.setAttribute("cy",d(t.value)),a.setAttribute("r","4"),a.setAttribute("class","chart-point"),a.setAttribute("data-index",e),a.addEventListener("mouseenter",e=>this.showChartTooltip(e,t,v)),a.addEventListener("mouseleave",()=>this.hideChartTooltip(v)),m.appendChild(a)}),g.appendChild(m);var E=document.createElementNS("http://www.w3.org/2000/svg","g");for(let e=0;e<=5;e++){var S=t+(r-t)*(e/5),C=document.createElementNS("http://www.w3.org/2000/svg","text");C.setAttribute("x",a.left-10),C.setAttribute("y",a.top+n-e/5*n+4),C.setAttribute("text-anchor","end"),C.setAttribute("class","chart-axis-text"),C.textContent=u.formatY?u.formatY(S):S.toFixed(1),E.appendChild(C)}g.appendChild(E),v.appendChild(g);w=document.createElement("div");return w.className="chart-tooltip",v.appendChild(w),g}v.innerHTML='<div class="chart-error">No valid data points</div>'}else v.innerHTML='<div class="chart-error">No data available</div>'}showChartTooltip(e,t,a){a=a.querySelector(".chart-tooltip");a.innerHTML=`
        ...
        `}renderGameCard(e,t){var a,i,n,s;return this.featureFlags.mobile_card_v2?this.renderMobileGameCardV2(e,t):(a=this.getSignalIcon(e.primarySignal),i=e.hasRecommendation,n=this.formatGameTime(e.datetime),s=this.formatOddsCompact(e),`
            <div class="game-card" 
                 data-game-index="${t}"
                ...
                <div class="game-row">
                    ...
                    <div class="game-odds mobile-hidden">${s}</div>
                    <div class="expand-btn">${i?"▼":""}</div>
                </div>
                
                ${i?this.renderExpandedContent(e):""}
            </div>
        `)}renderMobileGameCardV2(e,t){var a=e.hasRecommendation,i=this.formatGameTime(e.datetime),{homeChip:n,awayChip:s}=this.renderAhChips(e),r=this.renderEvPill(e);return`
                ...
                <div class="game-row game-row--v2">
                    <div class="game-meta">
                        ...
                        <div class="game-league">
                            <span class="league-flag">${e.leagueFlag}</span>
                            <span class="league-short">${e.leagueShort}</span>
                        </div>
                    </div>
        ...
        `}renderAhChips(e){var t,a,i,n,s,r,l;return e.hasAhData?(t=e=>0===e?"PK":0<e?"+"+e:(""+e).replace("-","−"),a=e.hasRecommendation&&e.recommendedTeam===e.home,i=e.hasRecommendation&&e.recommendedTeam===e.away,n=["ah-chip",e.homeLine<0?"ah-chip--neg":0<e.homeLine?"ah-chip--pos":"ah-chip--pk",a?"ah-chip--selected":""].filter(Boolean).join(" "),s=["ah-chip",e.awayLine<0?"ah-chip--neg":0<e.awayLine?"ah-chip--pos":"ah-chip--pk",i?"ah-chip--selected":""].filter(Boolean).join(" "),r=`Home ${t(e.homeLine)} Asian handicap`+(a?", recommended":""),l=`Away ${t(e.awayLine)} Asian handicap`+(i?", recommended":""),{homeChip:`
        ...
        `}):{homeChip:'<span class="ah-chip ah-chip--empty" aria-label="No Asian handicap data">AH —</span>',awayChip:'<span class="ah-chip ah-chip--empty" aria-label="No Asian handicap data">AH —</span>'}}renderEvPill(e){if(!e.hasRecommendation||!e.ev)return"";var e=(100*e.ev).toFixed(1),t=parseFloat(e);let a="ev-pill";return`<span class="${a+=25<=t?" ev-pill--strong":10<=t?" ev-pill--medium":" ev-pill--neutral"}" title="Expected Value: ${e}%" aria-label="Expected value ${e} percent">EV ${e}%</span>`}getSignalIcon(e){return{"kings-call":"👑","high-ev":"📈","hot-pick":"🔥","value-bet":"💎"}[e]||""}getSignalTitle(e){return{"kings-call":"King's Call Available","high-ev":"High Expected Value","hot-pick":"Hot Pick - High Confidence","value-bet":"Value Bet Opportunity"}[e]||""}formatGameTime(e){return e?e.toLocaleTimeString("en-GB",{hour:"2-digit",minute:"2-digit",hour12:!1}):"--:--"}formatOddsCompact(e){return e.hasRecommendation&&e.line&&e.recOdds?`AH ${0<=e.line?"+"+e.line.toFixed(2):e.line.toFixed(2)} @ `+e.recOdds.toFixed(2):e.odds1&&e.oddsX&&e.odds2?`${e.odds1.toFixed(2)} ${e.oddsX.toFixed(2)} `+e.odds2.toFixed(2):"TBD"}renderExpandedContent(e){return`
            <div class="expanded-details hidden">
                ...
                ${!e.kingsCall||e.kingsCall.includes("Unable to fetch")||e.kingsCall.includes("API error")?"":`
                    <div class="expanded-row kings-call-row">
                ...
                <div class="expanded-row stats-row">
                    <div class="stats-left">
                        <span class="stat-pill">Confidence: ${e.confidence}</span>
        ...
        `}format1X2Odds(e){return e.odds1&&e.oddsX&&e.odds2?`${e.odds1.toFixed(2)} ${e.oddsX.toFixed(2)} `+e.odds2.toFixed(2):"TBD"}initGameCardInteractions(){setTimeout(()=>{document.querySelectorAll('.game-card[data-expandable="true"]').forEach(t=>{t.addEventListener("click",e=>{e.target.closest(".expanded-actions")||e.target.closest(".action-btn")||this.toggleGameExpansion(t)})})},100)}toggleGameExpansion(e){var t=e.querySelector(".expanded-details"),a=e.querySelector(".expand-btn");t&&a&&(!t.classList.contains("hidden")?(t.classList.add("hidden"),a.textContent="▼",e.classList.remove("expanded"),e.setAttribute("data-expanded","false"),a.setAttribute("aria-expanded","false")):(t.classList.remove("hidden"),a.textContent="▲",e.classList.add("expanded"),e.setAttribute("data-expanded","true"),a.setAttribute("aria-expanded","true"),this.trackEvent("card_expanded",{signal:e.dataset.signal,league:e.querySelector(".league-short")?.textContent||e.querySelector(".game-league")?.textContent,tier:e.dataset.tier,has_pick:""!==e.dataset.signal,ev_band:this.getEvBand(e)})))}getEvBand(e){e=e.querySelector(".ev-pill");return e?e.classList.contains("ev-pill--strong")?"high":e.classList.contains("ev-pill--medium")?"medium":"low":"none"}shareGame(t){var e=this.data.unifiedGames.find(e=>e.datetime.getTime()===t);e&&(e=e.hasRecommendation?`🎯 ${e.recText} @ ${e.recOdds.toFixed(2)} odds (EV: ${(100*e.ev).toFixed(1)}%) - ${e.home} vs `+e.away:`⚽ ${e.home} vs ${e.away} - `+this.formatDateTime(e.datetime),navigator.share?navigator.share({title:"ParlayKing Betting Pick",text:e,url:window.location.href}):navigator.clipboard.writeText(e).then(()=>{alert("Shared text copied to clipboard!")}))}exportGame(t){var e,a=this.data.unifiedGames.find(e=>e.datetime.getTime()===t);a&&a.hasRecommendation&&(e=[["Date/Time","League","Match","Recommendation","Odds","EV (%)","Confidence","King's Call"],[this.formatDateTime(a.datetime),a.league,a.home+" vs "+a.away,a.recText,a.recOdds.toFixed(2),(100*a.ev).toFixed(1)+"%",a.confidence,a.kingsCall]].map(e=>e.join(",")).join("\n"),this.downloadCSV(e,`pick_${a.home.replace(/\s+/g,"")}_vs_${a.away.replace(/\s+/g,"")}.csv`))}formatNumber(e){return(new Intl.NumberFormat).format(e)}formatCurrency(e,t=!1){t=t&&0<=e?"+":"";return 1e3<=Math.abs(e)?t+(e/1e3).toFixed(1)+"k":t+e.toFixed(2)}formatDate(e){e=e instanceof Date?e:this.parseDateTimeSafe(e);return e?new Intl.DateTimeFormat("en-US",{month:"short",day:"numeric",timeZone:"Asia/Singapore"}).format(e):"--"}formatDateTime(e){e=e instanceof Date?e:this.parseDateTimeSafe(e);return e?new Intl.DateTimeFormat("en-US",{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0,timeZone:"Asia/Singapore"}).format(e):"--"}formatTimeAgo(e){var t;return e?(e=new Date(e),t=new Date,(t=Math.floor((t-e)/36e5))<1?"Just now":t<24?t+"h ago":Math.floor(t/24)+"d ago"):"--"}showLoading(e){var t=document.getElementById("loading-overlay");e?t.classList.remove("hidden"):t.classList.add("hidden")}showError(e){console.error(e)}initAnalyticsPage(){this.showLoading(!0),this.loadAllData().then(()=>{var e=document.getElementById("tier-select"),e=(e&&e.addEventListener("change",()=>this.renderROIHeatmap()),document.querySelectorAll(".range-btn[data-min-bets]").forEach(e=>{e.addEventListener("click",e=>{document.querySelectorAll(".range-btn[data-min-bets]").forEach(e=>e.classList.remove("active")),e.currentTarget.classList.add("active"),this.renderROIHeatmap()})}),document.getElementById("segments-tier-select"));e&&e.addEventListener("change",()=>this.renderTopSegments()),document.querySelectorAll(".toggle-btn[data-view]").forEach(e=>{e.addEventListener("click",e=>{var t=e.currentTarget.parentElement;t&&(t.querySelectorAll(".toggle-btn").forEach(e=>e.classList.remove("active")),e.currentTarget.classList.add("active"),this.renderPnLChart())})}),this.renderROIHeatmap(),this.renderPnLChart(),this.renderTopSegments(),this.updateLastRunStatus(),this.showLoading(!1)}).catch(e=>{console.error("Failed to load analytics data:",e),this.showError("Failed to load analytics data"),this.showLoading(!1)})}renderROIHeatmap(){var e=document.getElementById("roi-heatmap");if(e){let t=parseInt(document.querySelector(".range-btn.active[data-min-bets]")?.dataset.minBets||"30",10),r=parseInt(document.getElementById("tier-select")?.value||"1",10);let l=(Array.isArray(this.data.roiHeatmap)&&0<this.data.roiHeatmap.length?this.data.roiHeatmap.map(e=>({tier:+e.tier,line:+e.line,roi_pct:+e.roi_pct,n:+e.n})):this.getBacktestROIHeatmapFallback()).filter(e=>e.tier===r&&e.n>=t);var a=[...new Set(l.map(e=>e.line))].sort((e,t)=>e-t);if(0===a.length)e.innerHTML='<div class="chart-placeholder">No segments meet the selected filters</div>';else if(a.length<4){let s='<div class="segments-pills">';a.forEach(t=>{var e=l.find(e=>e.line===t)||{roi_pct:0,n:0},a=e.roi_pct,i=this.classForRoi(a),n=this.createSparklineFromRoi(a,`hm-${r}-`+t);s+=`
                    ...
                    </div>`}),s+="</div>",void(e.innerHTML=s)}else{let n=`<div class="heatmap-grid" style="grid-template-columns: repeat(${Math.max(a.length,1)}, 1fr);">`;a.forEach(t=>{var e=l.find(e=>e.line===t)||{roi_pct:0,n:0},a=e.roi_pct,i=10<=a?"heat-pos":3<=a?"heat-mid":"heat-neg";n+=`
